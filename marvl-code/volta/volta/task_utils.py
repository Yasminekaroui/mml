...
def EvaluatingModel(config, task_cfg, device, task_id, batch, model, dataloader, criterion, results, others):
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        for i in range(preds.size(0)):
            results.append(
                {
                    "id": str(question_id[i].item()),
                    "sentence": dataloader.dataset.entries[question_id[i].item()]["sentence"], # " ".join(dataloader.dataset._tokenizer.convert_ids_to_tokens(question[i], skip_special_tokens=True)),
