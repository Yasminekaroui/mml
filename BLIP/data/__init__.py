...
from data.multi30k_dataset import multi30k_retrieval
from data.vqa_dataset import vqa_dataset
from data.nlvr_dataset import nlvr_dataset
from data.marvl_dataset import marvl_dataset
...
def create_dataset(dataset, config, args, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        train_dataset = flickr30k_train(transform=transform_train, image_root=config['image_root'], ann_root=config['ann_root'], en=config['eng']) 
        val_dataset = flickr30k_retrieval_eval(transform=transform_test, image_root=config['image_root'], ann_root=config['ann_root'], split='val', en=config['eng']) 
        test_dataset = flickr30k_retrieval_eval(transform=transform_test, image_root=config['image_root'], ann_root=config['ann_root'], split='test', en=config['eng'] )          
        return train_dataset, val_dataset, test_dataset

    elif dataset=='retrieval_multi30k':          
        ...
        train_dataset = nlvr_dataset(transform_train, config['image_root'], config['ann_root'],split='train', lan=args.lan)
        val_dataset = nlvr_dataset(transform_test, config['image_root'], config['ann_root'],split='val', lan=args.lan)
        test_dataset = nlvr_dataset(transform_test, config['image_root'], config['ann_root'],split='test', lan=args.lan)     
        return train_dataset, val_dataset, test_dataset 
    
    elif dataset=='marvl': 
        id_dataset = marvl_dataset(transform_test, config['image_root'], config['ann_root'],'id', config['eng'])
        sw_dataset = marvl_dataset(transform_test, config['image_root'], config['ann_root'],'sw', config['eng'])
        ta_dataset = marvl_dataset(transform_test, config['image_root'], config['ann_root'],'ta', config['eng']) 
        tr_dataset = marvl_dataset(transform_test, config['image_root'], config['ann_root'],'tr', config['eng'])   
        zh_dataset = marvl_dataset(transform_test, config['image_root'], config['ann_root'],'zh', config['eng'])   
        return id_dataset, sw_dataset, ta_dataset, tr_dataset, zh_dataset 
